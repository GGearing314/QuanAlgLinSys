...
pi = np.pi 
nMax=8
n = nMax #Number of qubits in the counting register
#Functions:
...
while n>0:
    ...
    ratios.append(abs(rho[0][0]/rho[1][1]))
    ...
    n-=1 #decrement counting qubits
    
sol = np.linalg.solve(A, bsrc)
ratios.reverse()
plt.plot(np.arange(1,nMax+1),ratios, label="Estimated Solution Ratio")  
plt.axhline(y =(sol[0]**2)/sol[1]**2, color = 'r', linestyle = '-',label="Exact Solution Ratio",alpha=0.5)   
plt.xlabel("n")
